...
server <- function(input, output, session) {
  ...
  output$ratingsTable = renderDT({
    ...
    select(competitorID, competitorName, rating, percentile) %>%
  ...
  output$ratingsChart = renderPlotly({
    combined = data$ratings %>%
      filter(day < max(data$ratings$day)) %>%
      mutate(day = max(data$ratings$day),
             regattaID = max(data$ratings$day)) %>%
      rbind(data$ratings) %>%
      rbind(data$pastRatings) %>%
      mutate(regattaID = as.numeric(regattaID))
    ...
    ggplotly(ggplot(data = combined, aes(x = regattaID,
                                         y = rating,
                                         color = reorder(competitorID, rating))) +
      geom_line()+
